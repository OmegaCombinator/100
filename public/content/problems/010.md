...
This is a complete Acorn proof of Euler's theorem for natural-number modular
congruence. For any nonzero natural number `n`, if `a` is coprime to `n`, then
`a` raised to Euler's totient of `n` is congruent to `1` modulo `n`.

The proof uses the reduced-residue list from `nat.nat_totient`. Multiplication
by `a` modulo `n` permutes the reduced residues, so the product of the reduced
residues is congruent to `a.pow(n.totient)` times that same product. The product
is coprime to `n`, so `cancel_coprime` gives the final congruence.

## Harness result

- Run: `2026-05-06-top100-010-euler-gpt55-072-0e6b911b-062502`
- Session: `902419845a04`
- Model: `gpt-5.5`
- Acorn: `0.1.72`
- Independent verification: `29/29 OK` against upstream `acornlib` snapshot `0e6b911b`
- Upstream PR: <https://github.com/acornprover/acornlib/pull/399>

## Acorn proof

```acorn
from list.list_base import List, map
from list.list_permutation import is_permutation, permutation_preserves_product
from list.list_product import product
from nat import Nat
from nat.nat_congruence import congr_mod_mul, congr_mod_refl, congr_mod_symm
from nat.nat_modular_inverse import cancel_coprime
from nat.nat_totient import coprime_residues, coprime_residues_length, mul_mod_fn,
