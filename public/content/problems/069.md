# #69 Greatest Common Divisor Algorithm

Status: passed.

## Formalization

This is a complete Acorn wrapper proof for the Euclidean algorithm on natural numbers. It proves that for any `a b : Nat`, some Euclidean iterate stops with second component `0`, the first component is `a.gcd(b)`, and that value satisfies the usual greatest-common-divisor divisibility property.

## Harness result

- Run: `2026-05-01-top100-069-gcd-gpt55`
- Session: `2697229f64bc`
- Model: `gpt-5.5`
- Worker attempts: 2
- Independent verification: OK with Acorn `0.1.68`

## Acorn proof

```acorn
from nat.nat_base import Nat
from nat.nat_gcd import gcd_pair, gcd_step_n, num_gcd_steps
from pair import Pair

/// Freek Top 100 #69: the Euclidean algorithm computes greatest common divisors.
theorem top100_069_greatest_common_divisor_algorithm(a: Nat, b: Nat) {
    exists(n: Nat) {
        gcd_step_n(Pair.new(a, b), n).second = Nat.0 and
        gcd_step_n(Pair.new(a, b), n).first = a.gcd(b) and
        a.gcd(b).divides(a) and
        a.gcd(b).divides(b) and
        forall(d: Nat) {
            d.divides(a) and d.divides(b) implies d.divides(a.gcd(b))
        }
    }
} by {
    let n = num_gcd_steps(Pair.new(a, b))
    gcd_step_n(Pair.new(a, b), n).second = Nat.0
    gcd_step_n(Pair.new(a, b), n).first = gcd_pair(Pair.new(a, b))
    gcd_pair(Pair.new(a, b)) = a.gcd(b)
    gcd_step_n(Pair.new(a, b), n).first = a.gcd(b)
    a.gcd(b).divides(a)
    a.gcd(b).divides(b)
    forall(d: Nat) {
        if d.divides(a) and d.divides(b) {
            d.divides(a.gcd(b))
        }
    }
}
```

